...
namespace detail { 
  ...
  OSOptionalQuantity IdfObject_Impl::getQuantity(unsigned index,
                                                 bool returnDefault,
                                                 bool returnIP) const
  {
  ...
  bool IdfObject_Impl::isObjectListField(unsigned index) const {
    if (index >= numFields()) { return false; }
  ...
  UnsignedVector IdfObject_Impl::requiredFields() const {
    ...
    for (unsigned index = 0; index < m_fields.size(); ++index) {
      ...
      if (field && field->properties().required) { result.push_back(index); }
    }
    return result;
  }
  ...
  bool IdfObject_Impl::objectListFieldsEqual(const IdfObject& other) const {
    if (m_iddObject != other.iddObject()) { return false; }
  ...
  void IdfObject_Impl::parse(const std::string& text,bool getIddFromFactory)
  {
    ...
    if (boost::regex_search(parsedText, matches, idfRegex::line())){
      ...
      std::string commentOrOtherText(matches[2].first, matches[2].second); boost::trim_left(commentOrOtherText);
  ...
  void IdfObject_Impl::parseFields(const std::string& text)
  {
    ...
    while (boost::regex_search(start, stop, matches, idfRegex::line())) {
      ...
      if (commentOrOtherText.empty() ||
          boost::regex_match(commentOrOtherText, idfRegex::commentOnlyLine()))
      {
        // reduce the text
      ...
      if (iddField) {
        ...
        if (!commentOrOtherText.empty()) {
          ...
          if (!boost::regex_match(commentOrOtherText,
                                  commentRegex::editorCommentWhitespaceOnlyLine()))
          {
        ...
        LOG(Error, "IdfObject of type '" << m_iddObject.name() << "' " <<
          ...
          "remaining: " << std::endl << fieldText << std::endl << std::string (start, stop));
        return;
      }
...
void IdfObject_Impl::populateValidityReport(ValidityReport& report, bool checkNames) const
  {
    ...
    if (report.level() > StrictnessLevel::Draft) {
      ...
      if ((numFields() < minFields()) ||
          (maxFields() && (numFields() > maxFields().get())))
      {
        report.insertError(DataError(getObject<IdfObject>(),
                                     DataErrorType(DataErrorType::NumberOfFields)));
      }
  ...
  std::vector<DataError> IdfObject_Impl::fieldDataIsValid(unsigned index, const StrictnessLevel& level) const {
    ...
    if (!m_iddObject.getField(index)) {
      result.push_back(DataError(index,
                                 getObject<IdfObject>(),
                                 DataErrorType(DataErrorType::NoIdd)));
    ...
    if (level > StrictnessLevel::Draft) {
      ...
      if (!fieldIsNonnullIfRequired(index)) {
        result.push_back(DataError(index,
                                   getObject<IdfObject>(),
                                   DataErrorType(DataErrorType::NullAndRequired)));
      }
    } // end StrictnessLevel::Final
  ...
  bool IdfObject_Impl::withinBounds(double fieldValue,const IddField& iddField) const {
    ...
    if (boundType != IddFieldProperties::Unbounded) {
      ...
      else if (fieldValue <= *boundValue) { return false; }
    }
  ...
  OSOptionalQuantity IdfObject_Impl::getQuantityFromDouble(unsigned index, boost::optional<double> value, bool returnIP) const {
    ...
    if (!iddField) {
      LOG_AND_THROW("get/setQuantity not available without an IddField. Asked to getQuantity at "
                    "field " << index << "for IdfObject with Idd:\n" << m_iddObject);
    }
...
boost::optional<std::string> IdfObject::getString(unsigned index,
                                                  bool returnDefault,
...
OSOptionalQuantity IdfObject::getQuantity(unsigned index,
                                                 bool returnDefault,
                                                 bool returnIP) const
{
