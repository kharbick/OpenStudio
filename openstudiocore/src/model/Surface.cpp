...
namespace detail {
 ...
 std::vector<ModelObject> Surface_Impl::children() const
 {
  ...
  SubSurfaceVector Surface_Impl::subSurfaces() const
  {
    ...
    WorkspaceObjectVector idfSubSurfaces = object.getSources(IddObjectType(SubSurface::iddObjectType()));
  ...
  bool Surface_Impl::setAdjacentSurface(Surface& surface) {
    ...
    if (test){
      ...
      for (WorkspaceObject wo : this->getSources(iddobjectname::OS_Surface)){
      ...
      if (!isSameSurface){
        ...
        for (WorkspaceObject wo : surface.getSources(iddobjectname::OS_Surface)){
